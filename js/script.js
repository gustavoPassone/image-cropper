...
const postProcessingToolbar = document.getElementById("post-processing-toolbar");
...
function handleSingleFile(file) {
  ...
  if (file.size > MAX_FILE_SIZE_BYTES) {
    alert(`Este arquivo é muito grande (Limite: ${MAX_FILE_SIZE_MB}MB) e pode travar seu navegador.`);
...
function handleMultipleFiles(files) {
    ...
    if (imageFiles.length !== files.length) {
        alert('Por favor, selecione apenas arquivos de imagem (JPG, PNG) para o processamento em lote.');
        return;
    }

    let totalSize = imageFiles.reduce((acc, file) => acc + file.size, 0);
    if (totalSize > MAX_FILE_SIZE_BYTES * 2) { // Limite maior para lote
        alert(`O tamanho total dos arquivos é muito grande e pode travar seu navegador.`);
        return;
    }
...
async function loadPdfPageIntoEditor(pageNum) {
  ...
  try {
    ...
    originalImage.onerror = () => alert("Não foi possível carregar a imagem do PDF.");
  } catch (error) {
...
function loadImageIntoEditor(index) {
    ...
    originalImage.src = URL.createObjectURL(file);
    originalImage.onload = showEditScreen;
    originalImage.onerror = () => alert("Não foi possível carregar a imagem selecionada.");
}
...
function initializeCanvas() {
  ...
  let scaleFactor = Math.min(1, maxWidth / originalRotatedWidth, maxHeight / originalRotatedHeight);
  ...
  if (autoPoints && autoPoints.length === 4) {
    points = autoPoints.map(p => ({ x: p.x * scaleFactor, y: p.y * scaleFactor }));
    if (points.some(p => p.x > displayedImageWidth || p.y > displayedImageHeight || p.x < 0 || p.y < 0)) {
...
function drawCanvas() {
  ...
  const isRotated90or270 = rotationAngle % 180 !== 0;
  let imageWidth = isRotated90or270 ? displayedImageHeight : displayedImageWidth;
  let imageHeight = isRotated90or270 ? displayedImageWidth : displayedImageHeight;
  editCtx.drawImage(originalImage, -imageWidth / 2, -imageHeight / 2, imageWidth, imageHeight);
  editCtx.restore();
  ...
  points.forEach((p, index) => {
    ...
    editCtx.strokeStyle = index === draggingPoint ? DRAGGING_STROKE_COLOR : POINT_STROKE_COLOR;
    editCtx.lineWidth = LINE_WIDTH;
    editCtx.beginPath();
...
function updateLoupe(x, y) {
  ...
  loupeCtx.drawImage(
    editCanvas,
    sourceX, sourceY,       // Posição do retângulo de origem
    sourceSize, sourceSize, // Tamanho do retângulo de origem
    0, 0,                   // Posição do retângulo de destino
    LOUPE_SIZE, LOUPE_SIZE  // Tamanho do retângulo de destino
  );
...
function findDocumentCorners(imageElement) {
  ...
  let src; let gray; let blurred; let edged; let contours; let hierarchy; let bestApprox;
  try {
    src = cv.imread(imageElement); gray = new cv.Mat(); cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY);
    blurred = new cv.Mat(); cv.GaussianBlur(gray, blurred, new cv.Size(5, 5), 0, 0, cv.BORDER_DEFAULT);
    edged = new cv.Mat(); cv.Canny(blurred, edged, 75, 200);
    ...
    for (let i = 0; i < contours.size(); ++i) {
      ...
      if (area > (src.rows * src.cols) / 20) {
        ...
        if (approx.rows === 4 && area > maxArea) {
          maxArea = area; if (bestApprox) { bestApprox.delete(); }
          bestApprox = approx;
        } else { approx.delete(); }
      }
      cnt.delete();
    ...
    if (bestApprox) {
      let pts = [
        { x: bestApprox.data32S[0], y: bestApprox.data32S[1] }, { x: bestApprox.data32S[2], y: bestApprox.data32S[3] },
        { x: bestApprox.data32S[4], y: bestApprox.data32S[5] }, { x: bestApprox.data32S[6], y: bestApprox.data32S[7] },
      ];
      pts.sort((a, b) => a.y - b.y);
      let top = pts.slice(0, 2).sort((a, b) => a.x - b.x); let bottom = pts.slice(2, 4).sort((a, b) => a.x - b.x);
      let orderedPoints = [top[0], top[1], bottom[1], bottom[0]];
      bestApprox.delete(); return orderedPoints;
    } return null;
  } catch (error) { console.error("Auto-crop falhou:", error); return null;
  } finally {
    if (src) src.delete(); if (gray) gray.delete(); if (blurred) blurred.delete();
    if (edged) edged.delete(); if (contours) contours.delete(); if (hierarchy) hierarchy.delete();
...
function getWarpedResult() {
  const origW = originalImage.width, origH = originalImage.height;
  const rotatedDims = getRotatedDimensions();
  const originalRotatedWidth = rotatedDims.width, originalRotatedHeight = rotatedDims.height;
  const originalToDisplayedScaleX = originalRotatedWidth / displayedImageWidth;
  const originalToDisplayedScaleY = originalRotatedHeight / displayedImageHeight;
  const pointsOnRotatedOriginal = points.map(p => ({ x: p.x * originalToDisplayedScaleX, y: p.y * originalToDisplayedScaleY }));

  let finalPointsOriginalScale = [];
  pointsOnRotatedOriginal.forEach(p => {
    let x, y;
    if (rotationAngle === 90) { x = p.y; y = origH - p.x; }
    else if (rotationAngle === 180) { x = origW - p.x; y = origH - p.y; }
    else if (rotationAngle === 270) { x = origW - p.y; y = p.x; }
    else { x = p.x; y = p.y; }
    finalPointsOriginalScale.push(x, y);
  });
  
  const p0 = { x: finalPointsOriginalScale[0], y: finalPointsOriginalScale[1] }, p1 = { x: finalPointsOriginalScale[2], y: finalPointsOriginalScale[3] };
  const p2 = { x: finalPointsOriginalScale[4], y: finalPointsOriginalScale[5] }, p3 = { x: finalPointsOriginalScale[6], y: finalPointsOriginalScale[7] };
  const w1 = Math.hypot(p0.x - p1.x, p0.y - p1.y), w2 = Math.hypot(p3.x - p2.x, p3.y - p2.y);
  const h1 = Math.hypot(p0.x - p3.x, p0.y - p3.y), h2 = Math.hypot(p1.x - p2.x, p1.y - p2.y);
  const outWidth = Math.round(Math.max(w1, w2)), outHeight = Math.round(Math.max(h1, h2));
  
  let src = cv.imread(originalImage);
  let srcPoints = cv.matFromArray(4, 1, cv.CV_32FC2, finalPointsOriginalScale);
  let dstPoints = cv.matFromArray(4, 1, cv.CV_32FC2, [0, 0, outWidth - 1, 0, outWidth - 1, outHeight - 1, 0, outHeight - 1]);
  let M = cv.getPerspectiveTransform(srcPoints, dstPoints);
  let dst = new cv.Mat();
  let dsize = new cv.Size(outWidth, outHeight);
  cv.warpPerspective(src, dst, M, dsize, cv.INTER_LINEAR, cv.BORDER_CONSTANT, new cv.Scalar());
  
  src.delete(); M.delete(); srcPoints.delete(); dstPoints.delete();
  return dst;
}
...
function handleExport() {
    const format = exportFormatSelect.value; const filename = exportFilenameInput.value.trim();
    if (!filename) { alert("Por favor, insira um nome para o arquivo."); exportFilenameInput.focus(); return; }
    if (format === 'pdf') {
        try {
            const imgData = resultCanvas.toDataURL("image/png"); const w = resultCanvas.width; const h = resultCanvas.height;
            const orientation = w > h ? "l" : "p"; const { jsPDF } = window.jspdf;
            const pdf = new jsPDF({ orientation, unit: "px", format: [w, h] });
            pdf.addImage(imgData, "PNG", 0, 0, w, h); pdf.save(`${filename}.pdf`);
        } catch (e) { console.error("Erro ao gerar PDF:", e); alert("Não foi possível gerar o PDF."); }
    } else {
        const link = document.createElement("a"); link.href = resultCanvas.toDataURL("image/png");
        link.download = `${filename}.png`; link.click(); setTimeout(() => link.remove(), 100);
    }
}
...
function updateActiveFilterButton(activeFilter) {
    ...
    const filterButtons = [filterNoneButton, filterBwButton, filterContrastButton, filterSharpenButton, filterMagicButton];
    const buttonMap = {
    ...
    if (buttonMap[activeFilter]) {
        ...
        if (activeFilter !== 'magic') {
            activeBtn.classList.remove('bg-[#30363D]', 'text-[#C9D1D9]', 'hover:bg-[#3d444c]');
...
function applyPostProcessingEffects() {
    ...
    try {
        if (currentFilter === 'magic') {
            ...
            cv.adaptiveThreshold(gray, matToDisplay, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 11, 4);
            gray.delete();
            ...
            cv.adaptiveThreshold(gray, matToDisplay, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 11, C);
            gray.delete();
            ...
            cv.filter2D(correctedImageMat, sharpened, cv.CV_8U, kernel, new cv.Point(-1, -1), 0, cv.BORDER_DEFAULT);
            
            const alpha = parseInt(intensitySlider.value, 10) / 100;
            const beta = 1.0 - alpha;
            
            matToDisplay = new cv.Mat();
            cv.addWeighted(sharpened, alpha, correctedImageMat, beta, 0, matToDisplay, -1);
...
function applyCleanup() {
    if (brushPath.length === 0 || !correctedImageMat || correctedImageMat.isDeleted()) {
    ...
    try {
        ...
        mask = new cv.Mat.zeros(correctedImageMat.rows, correctedImageMat.cols, cv.CV_8UC1);
        for (let i = 1; i < brushPath.length; i++) {
            const p1 = new cv.Point(Math.round(brushPath[i-1].x), Math.round(brushPath[i-1].y));
            const p2 = new cv.Point(Math.round(brushPath[i].x), Math.round(brushPath[i].y));
        ...
        cv.adaptiveThreshold(gray, processedRegion, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 11, 4);
...
function resetToUploadScreen() {
  fileInput.value = null; originalImage = null; originalFileName = "";
  originalFileType = "image/png"; points = []; draggingPoint = null; rotationAngle = 0;
...
function handleModalKeydown(e) {
  if (e.key === "Enter") { e.preventDefault(); modalBtnYes.click(); }
  else if (e.key === "Escape") { e.preventDefault(); modalBtnNo.click(); }
}
function showModal() {
...
modalBtnYes.addEventListener("click", () => { hideModal(); resetToUploadScreen(); });
...
filterMagicButton.addEventListener('click', () => updateActiveFilterButton('magic'));
filterNoneButton.addEventListener('click', () => updateActiveFilterButton('none'));
filterBwButton.addEventListener('click', () => updateActiveFilterButton('bw'));
filterContrastButton.addEventListener('click', () => updateActiveFilterButton('contrast'));
filterSharpenButton.addEventListener('click', () => updateActiveFilterButton('sharpen'));
...
function getResultCanvasPos(e) {
    ...
    const canvasX = ((mouseX / rect.width) * resultCanvas.width - resultPan.x) / resultZoom;
    const canvasY = ((mouseY / rect.height) * resultCanvas.height - resultPan.y) / resultZoom;
